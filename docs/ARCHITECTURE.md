...
The Smart Link creation form accepts any user-entered or pasted URL, including protocol-free values such as `google.com`, and normalizes the protocol server-side. It also provides an optional selector for existing shortened links, which inserts the selected short URL as the new destination while preserving the current custom slug workflow.
