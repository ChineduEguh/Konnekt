...
The weekly digest flow now exposes a cron-authenticated `POST /api/scheduled/weeklyDigest` callback. A workspace stores its Heartbeat task UID in `scheduleCronTaskUid`, and the protected `workspace.scheduleWeeklyDigest` mutation creates the six-field UTC Heartbeat job and persists that UID. The callback looks up the workspace by the authenticated cron task UID, compiles the current seven-day summary, returns an idempotent orphan response when the task has no owner, and reports structured diagnostics on failure. The callback now sends the prepared digest through the built-in owner notification channel and returns the delivery result. The production job should be activated only after this checkpoint is deployed and the callback is verified.
...
QR Studio stores dynamic QR assets in `qrCodes`, each bound to a smart link. The client uses the QR encoding library to render a live PNG preview with configurable foreground colour, gradient foreground end colour, background colour, module shape presentation, corner-eye shape, font, frame style, and frame label. PNG and PDF exports compose the QR, border, and CTA label into a single image. SVG exports include the same gradient definition, logo, frame border, and CTA label. The destination remains the smart link redirect path so it can be edited later without reprinting the physical asset.
...
The analytics page aggregates QR-origin scan events by day and provides date windows for 7, 30, and 90 days. The protected `analytics.trends` procedure returns tenant-scoped daily smart-link click and QR scan series for the selected range and the preceding equivalent range. The dashboard renders both lines, exact hover values, a comparison filter, current and previous totals, and a percentage trend indicator. It also displays current smart-link count, all-time link clicks, and all-time QR scan activity. CSV export downloads a portable file directly. The Google Sheets action copies tab-separated data to the clipboard, shows a preview modal, reports success feedback, and opens a new Sheet so the user can paste the dataset without exposing credentials to the application. Authenticated one-click Sheets API creation remains a future provider-dependent roadmap item. Workspace Settings exposes a persistent preparation toggle that records intent without changing the current clipboard workflow. Published events are available through public `/events/:eventId` pages. Public detail queries expose only published events and registration counts. Public registration validates event status, workspace context, capacity, and duplicate attendee email before issuing a ticket. Event management provides the same direct CSV download pattern for attendee records. CRM contact profiles now have tenant-scoped list and create procedures, and their WhatsApp message timeline is available through the protected conversations procedures.

## Shared icon consistency

Workspace icon presentation is governed by a shared CSS layer in `client/src/index.css`. Metric, activity, link, card, chart, empty-state, navigation, and theme-toggle icons use consistent Lucide dimensions, stroke weights, rounded containers, border contrast, and semantic colour tokens. The `:root` tokens provide light-mode pairs, while `.dark` overrides provide darker surfaces and brighter foregrounds without relying on hard-coded light-only colours. The audit notes are recorded in `docs/icon-audit.md`.
...
QR Studio keeps the preview visible as a distinct section below the controls on phones. Uploaded logos are composited into the QR canvas before the preview and download data URL are updated. The preview identifies whether a centre logo is active and reports an actionable error if the selected image cannot be decoded. The custom frame is rendered around the preview and is also included in PNG, SVG, PDF, and native-share image outputs.
