...
Events support registration, ticket codes, capacity controls, attendee search, status filters, CSV export, and protected QR check-in. Customers support tenant-scoped profiles and unified activity timelines combining event registrations and WhatsApp messages. Analytics provides daily QR scan trends, exact chart tooltips, geography, devices, browsers, top links, conversion funnel views, CSV export, and a Google Sheets-compatible clipboard export.
